...
def avg_per_batch(session: orm.Session, sensor: str) -> sqla.sql.expression.CTE:
    ...
    cte = (
        ...
        .join(BatchPhase, BatchPhase.start_date.between(BatchInfo.start_date, BatchInfo.end_date))
        .join(
            sensor_table,
        ...
        .group_by(
            BatchInfo.batch_id,
            BatchPhase.end_date - BatchPhase.start_date
        )
        .cte()
    )
    return cte
...
def get_batch_sensor_difference(
    ...
    query = (
        session.query(
            BatchInfo.id,
            sqla.func.date_trunc('minutes', _round_timestamp(sensor_table_2.timestamp)).label('date'),
            (sensor_table_2.value - sensor_table_1.value).label(f'{sensor_type}_difference')
        )
        .join(BatchPhase, BatchPhase.start_date.between(BatchInfo.start_date, BatchInfo.end_date))
        .join(
            sensor_table_2,
            sensor_table_2.timestamp.between(BatchInfo.start_date, BatchInfo.end_date)
        )
        .join(
            sensor_table_1,
            (
                sqla.func.date_trunc('minutes', _round_timestamp(sensor_table_1.timestamp)) ==
                sqla.func.date_trunc('minutes', _round_timestamp(sensor_table_2.timestamp))
            ),
        )
        .filter(BatchPhase.batch_phase == 'cultivation')
